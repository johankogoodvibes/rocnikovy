...
void solve_kempecycle(vector<vector<int>>& g, map<pair<int, int>, int>& solved, int a, int b, map<pair<int, int>, int>& last_visit) {
    if (a > b) swap(a, b);
    ...
    for (int c2 = 1; c2 <= 3; c2++) {
        ...
        for (auto i : res) {
            ...
            solve_kempecycle(g, solved, i.first, i.second, last_visit);
            apply_kempeswitch(kempe_switch_cycle, c3);
        }
    }
...
bool check_critical(vector<vector<int>>& g) {
    ...
    while (!edges.size() == 0) {
        ...
        solve_kempecycle(g, solved, a, b, last_visit);
